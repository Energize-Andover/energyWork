...
try:
    ...
    print('{0},{1},{2},{3},'.format('"Date / Time"', '"Name Path Reference"', '"Object Name"', '"Object Value"'))
    ''' co2_file.write('{0},{1},{2},{3},'.format('"Date / Time"', '"Name Path Reference"',
                                             '"Object Name"', '"Object Value"'))
    temp_file.write('{0},{1},{2},{3},'.format('"Date / Time"', '"Name Path Reference"',
                                              '"Object Name"', '"Object Value"')) '''
    ...
    for index, row in df.iterrows():
        ...
        # Output CO2 CSV file
        co2_file.write("\n")
        print('CO2 Levels - {0},{1},{2},{3}'.format(time.strftime("%m/%d/%Y %H:%M:%S"), co2_units, row['Facility'] + '-' + row['Label'], co2_value))
        co2_file.write('{0},{1},{2},{3}'.format(time.strftime("%m/%d/%Y %H:%M:%S"), co2_units, row['Facility'] + '-' + row['Label'], co2_value))

        # Output Temp CSV file
        temp_file.write("\n")
        print('Temperature - {0},{1},{2},{3},'.format(time.strftime("%m/%d/%Y %H:%M:%S"), temp_units,
                                       row['Facility'] + '-' + row['Label'], temp_value))
        temp_file.write('{0},{1},{2},{3},'.format(time.strftime("%m/%d/%Y %H:%M:%S"), temp_units,
                                                row['Facility'] + '-' + row['Label'], temp_value))
